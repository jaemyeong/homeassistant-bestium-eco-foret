...
<head>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing:border-box; }
    ...
    .monitor-row span { display:block; font-size:var(--ha-font-size-s); line-height:var(--ha-line-height-condensed);
    ...
    .kv div { padding:var(--ha-space-3); background:var(--input-fill-color); border-radius:var(--ha-radius-md);
    ...
    .zone-head { display:flex; align-items:center; gap:var(--ha-space-2); }
...
<body>
  <main>
    ...
    <section class="card wide" aria-labelledby="gate-title" data-surface="control"><h2 id="gate-title">전송 게이트 · <span lang="en">Transmission gate</span></h2><div id="gate-banner" class="banner" data-state="unknown" role="status" aria-live="polite"><p id="gate-banner-title" class="banner-title">상태를 확인하는 중입니다 · <span lang="en">Checking status</span></p><p id="gate-banner-lede" class="banner-lede"></p><ul id="gate-banner-reasons" class="banner-reasons"></ul><p id="gate-banner-fix" class="banner-fix"></p></div><p id="trust-message" class="muted">미리보기는 연결 없이 가능합니다. <span lang="en">Preview only; live commit requires an authorized, connected, quiet current transport.</span></p><div class="actions" aria-label="Capture actions · 캡처 동작"><button id="capture-start" type="button" disabled>캡처 시작 · <span lang="en">Start capture</span></button><button id="capture-stop" class="secondary" type="button" disabled>중지 · <span lang="en">Stop</span></button><button id="capture-download" class="secondary" type="button" disabled>다운로드 · <span lang="en">Download</span></button></div></section>
    ...
    (() => {
      ...
      const draw = (payload) => {
        ...
        for (const entry of frames.slice(-32)) {
          ...
          const age = ageFor(entry, serverNow, currentGeneration, runtimePhase, staleAfterMs, source.lastValidFrameAtMs, source.lastValidFrameGeneration);
          item.textContent = String(entry.rawHex || entry.frameHex || "unknown") + " · " + (age === null ? "stale" : "age " + age + " ms · fresh") + " · generation " + displayGeneration(entry?.generation ?? source.generation);
