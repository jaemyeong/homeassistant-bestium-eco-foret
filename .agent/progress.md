...
| M0.3a | Serena's canonical project config is committed after fresh-activation normalization; restart handoff is repaired | Complete | Current signed `chore(m0): stabilize Serena project config` HEAD |
...
- Fresh clean-root activation with Serena 1.7.0 expanded the tracked two-key project file with its canonical defaults. The live process reports project `homeassistant-bestium-eco-foret`, the clean root, active `typescript`, and language-server status `ready`.
- The user authorized committing that canonical Serena output as M0.3a instead of repeatedly restoring an operationally unstable two-key file. The next fresh session must verify the signed M0.3a HEAD and a clean worktree before M0.4 begins.
- The read-only M0.3a adversarial audit found no actionable P0/P1/P2/P3 issue. The next fresh session must verify this signed repair HEAD and prove the canonical config is not rewritten.
