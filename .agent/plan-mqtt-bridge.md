...
**일괄소등**: 이 공백은 2026-09-01에 닫혔습니다. 운영자가 걸고 풀면서 월패드로 닿지 못하는
방들을 직접 확인했고, **해제하면 각 방의 점등 상태가 복원됩니다.** 걸기 전에 켜져 있던 방은
켜지고 꺼져 있던 방은 꺼진 채입니다. 그러므로 대칭 페이로드는 옳았고, 걸기는 되돌릴 수 있는
조작입니다.

**단 등 1은 이전 상태와 무관하게 켜집니다.** 이것은 `tools/buslab/guard.ts:93`이 손 조작 4회로
이미 기록해 둔 동작이었으나, 그 기록이 이 설계 판단에 쓰이지 않았습니다. 결과적으로
**해제를 씬에 넣으면 조명 하나를 켜고 나가게 됩니다.** 두 DOCS 파일이 그것을 말합니다.

조명 외에 무엇이 더 걸리는지는 여전히 관측되지 않았습니다.
